...
const Appeals = () => {
  ...
  return (
    <section className="container mx-auto my-4 relative">
      ...
      <div id="cards" className={`${Styles.cards} mx-auto`}>
        {AppealsCards.map((cards, idx) => {
          return <Cards cards={cards} idx={idx} key={idx}></Cards>;
        })}
      </div>
      <div>
        <div className={`${Styles.appealsButtons} text-center my-10`}>
          {AppealsCards.map((val, idx) => {
            return (
              <Button
                cardsNum={idx}
                currCards={currCards}
        ...
        <Link className="absolute -bottom-2 right-0 px-4 py-2 border rounded-md border-custom-gray-shade1 text-custom-blue-shade2 font-semibold" href="/appeals">View all</Link>
    </section>
  );
};
